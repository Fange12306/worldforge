...
<p align="center">
  <img src="https://img.shields.io/badge/version-0.6.7-blue?style=flat-square" alt="version" />
...
### 1. Fantasy Map System
Procedural fantasy map generation driven by your world data. Describe a continent or let the AI read your existing location entries — it outputs structured map parameters (regions, cities, rivers, terrain), and the generation engine fills in detailed heightmaps, biomes, river networks, road paths, and political boundaries. The result is stored as structured terrain data (area, distance matrix, adjacency table) that the AI agent queries via deterministic tools — so the LLM never has to guess about spatial relationships. Every generated feature auto-creates location entries, relations (`located_in`, `borders`, `capital_of`), and optional timeline events. Includes an interactive map viewer with layers (terrain, political, roads, markers) and click-through to entry panels.

### 2. Entry State Timeline
...
### 3. Character Personality Profiles
...
### 5. Living World Engine (Long-term Vision)
