...
function buildZhPrompt(
  ...
  return [
    ...
    `  1) Relation — 词条之间的静态关系，跨所有时间轴成立。如"张三是李四的父亲""精灵族起源于生命之树"。创建关系传 from/to/description，非对称关系必须传 reverse_description（如正向"父亲"→反向"子女"、正向"位于"→反向"容纳"、正向"女王"→反向"臣民"）。对称关系（战友、结盟、兄弟）不需要传。同两个词条可以有多条边（不同 description）。更新或删除前用 ExploreGraph 拿 relation_id。from_id/to_id 直接传词条名称即可，后端自动解析，无需复制 UUID。`,
    ...
    `- relationship_changes 为 JSON 数组：[{"entry_a":"uuid","entry_b":"uuid2","change_type":"add","relation":"战友","description":"共同抵御虚空入侵"}]。方向为 entry_a → relation → entry_b，即 entry_a 是 entry_b 的 relation（如"林逸风"→"担任舰长"→"黎明号"=林逸风是黎明号的舰长）。add 关联自动同步到 ExploreGraph（start_event_id 指向此事件），delete 关联标记 end_event_id。只有事件确实改变了词条间关系时才用。`,
...
function buildEnPrompt(
  ...
  return [
    ...
    `  1) Relation — static relationships between entries, valid across all timelines. E.g. "Zhang San is Li Si's father", "Elves originated from the Tree of Life". Create with from/to/description. For asymmetric relations, always provide reverse_description (e.g. forward "father" → reverse "child", forward "located in" → reverse "contains", forward "queen" → reverse "subject"). Symmetric relations (allies, siblings) don't need it. Same pair can have multiple edges (different description). Use ExploreGraph to get relation_id before update/delete. For from_id/to_id you can pass entry names directly — the backend resolves them automatically, no need to copy UUIDs.`,
    ...
    `- relationship_changes format: JSON array [{"entry_a":"uuid","entry_b":"uuid2","change_type":"add","relation":"ally","description":"saved in battle"}]. Direction: entry_a → relation → entry_b, i.e. entry_a is the relation of entry_b (e.g. "Lin"→"captain_of"→"Dawnbreaker" = Lin is the captain of Dawnbreaker). add auto-syncs to ExploreGraph (start_event_id points to this event), delete marks end_event_id. Only use when events actually change relationships between entries. The same pair can have multiple edges with different descriptions.`,
