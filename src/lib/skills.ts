...
export const BUILT_IN_SKILLS: BundledSkill[] = [
  ...
  // ── 3. chapter-outline ──
  {
    ...
    prompt: [
      ...
      `   - linked_events: 逗号分隔的 timeline_id:event_id（章节描绘的事件）`,
      `4. **建立关联** — 通过 linked_events 关联时间线事件；不要直接建立章节↔词条关系`,
      `5. **规划后续** — 建议下一章方向`,
    ].join("\n"),
    promptEn: [
      ...
      `   - linked_events: Comma-separated timeline_id:event_id values for events the chapter depicts`,
      `4. **Link** — Link timeline events through linked_events; do not create direct chapter↔entry relations`,
  ...
  // ── 6. export-ebook ──
  {
    ...
    prompt: [
      ...
      `2. **读内容** — 对 status 为 "done" 的章节逐一 OutlineRead(chapter_id)`,
      `3. **组装** — 将章节按顺序组织为完整文档：`,
      `   - 封面：世界名称 + 故事标题`,
      `   - 目录列表`,
      `   - 各章正文（保持原作者格式）`,
    ...
    promptEn: [
      ...
      `2. **Read content** — For chapters with status "done", OutlineRead(chapter_id) each one`,
