...
function buildZhPrompt(
  ...
  return [
    ...
    `- 需要用户拍板或关键信息缺失时（如事件的时间与概况、用户偏好、二选一的创作方向），用 AskUserQuestion 一次问清楚，而不是猜。可一次问多个问题，界面会逐个展示；尽量给 2-5 个选项，用户也可以输入自定义回答。提问要克制：能查证/读取的先查，能合理推断的不要问，问完立即按回答继续执行。`,
