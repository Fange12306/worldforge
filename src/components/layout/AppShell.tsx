  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      ...
      <div className="flex-1 overflow-auto">
        {view.type === "entry" ? (
          <EntryEditor key={view.entry.id} entry={view.entry} editing={view.editing} worldPath={worlds.find(w => w.id === activeWorldId)?.path || ""} onEdit={() => onUpdate({ ...view, editing: true })} onCancel={() => onUpdate({ ...view, editing: false })} onSave={async (e: Entry) => { try { const w = worlds.find((x) => x.id === activeWorldId); if (w) { const result = await invoke<Entry>("update_entry", { worldPath: w.path, entryId: e.id, name: e.name, body: e.body || "" }); onUpdate({ type: "entry", entry: result, editing: false }); } } catch {} }} onDelete={async () => { const w = worlds.find((x) => x.id === activeWorldId); if (w) { await invoke("delete_entry", { worldPath: w.path, entryId: view.entry.id }); onBack(); } }}
