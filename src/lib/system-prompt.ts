...
function buildZhPrompt(
  ...
  return [
    ...
    `  1) Relation — 词条之间的静态关系，跨所有时间轴成立。如"张三是李四的父亲""精灵族起源于生命之树"。创建关系传 from/to/description，非对称关系必须传 reverse_description（如正向"父亲"→反向"子女"、正向"位于"→反向"容纳"、正向"女王"→反向"臣民"）。对称关系（战友、结盟、兄弟）不需要传。同两个词条可以有多条边（不同 description）。更新或删除前用 ExploreGraph 拿 relation_id。`,
