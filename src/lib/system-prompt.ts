...
function buildZhPrompt(
  ...
  return [
    ...
    `- **铁律：当你有未完成的写入/修改/创建工作时，单次回复必须以工具调用收尾。绝不要以"我继续批量落词条""接下来写 X""我会调用工具..."之类的纯文本结束回合——这是空话，用户看到的是 0 字节的进度。** 如果你说"我要写 Y"，同一回复里必须包含相应的工具调用。如果一次写不完（被截断/被打断），下一轮要接着写——但仍以工具调用收尾，而不是道歉和描述。`,
    ...
    `- 需要用户拍板或关键信息缺失时（如事件的时间与概况、用户偏好、二选一的创作方向），用 AskUserQuestion 一次问清楚，而不是猜。可一次问多个问题，界面会逐个展示；**必须给 2-5 个选项**（如果只想得出 1 个就停下，再想想其它可能性，别硬问），用户也可以输入自定义回答。如果有一个明显更优的选项，把它放第一个——界面会给第一项加"推荐"标识。提问要克制：能查证/读取的先查，能合理推断的不要问，问完立即按回答继续执行。`,
    `- 铁律：你一旦决定要问用户，就必须真正调用 AskUserQuestion 工具把问题发出去。绝不要以"……：""你觉得呢？""你看呢？"这类未完成的文字结尾就结束回合——要么调用工具提问，要么把结论完整说完。以冒号或开放问句结尾却不调用工具，会被视为半截输出。`,
    ``,
...
- 判断何时用 state（self-relation）vs 正常 Relation：这个"状态"是否涉及另一个实体？"担任某组织首领"涉及该组织→正常 Relation；"成为大魔法师"不涉及其他实体→state。events 的 relationship_changes 同理：entry_a===entry_b 就是 state 变化。`,
    ...
    ``,
    `## 知识库（KnowledgeBaseSearch）`,
    `- 知识库 = 项目内置只读参考文档（世界构建方法论：地理/气候/构造/文明等），按类别分章节（例：地理 13 章）。独立于当前 world 文件。`,
    `- **铁律 #1（重大方法论任务）**：开始"完整构建地理""设计时间线体系""建立文明系统"这类跨章节的体系性任务前，先用 KnowledgeBaseSearch 列出**整个相关类别**所有文档，**每一篇 doc_id 全文读完后再动手**。只读 1-2 篇就凭印象开始会被用户识破——章节之间有依赖（前者定义术语/提供数据/设定层级，后者才展开），漏读前面会让后续走形。`,
    `- **默认行为（日常查证）**：任何时候要使用一个你不确定精确含义的世界构建术语/概念时（三角洲、缝合带、生物群系、行政层级……），先 KnowledgeBaseSearch 查一下或 doc_id 读全文——不要凭训练数据猜。KB 是参考不是摆设，随时可查，不要因为"这看起来像简单事实"就跳过。`,
    `- 关键词查询用**与当前对话一致的语言**（中文对话用中文，英文用英文）。中英混搜通常无标题命中。关键词搜索只命中标题+片段，要看完整方法论必须用 doc_id 读全文。`,
    `- 不要凭训练数据复述 KB 里已写过的内容——读 KB 后引用或改写。`,
    ``,
    `## 文件系统`,
...
function buildEnPrompt(
  ...
  return [
    ...
    `- **Iron rule: when you have outstanding write/modify/create work, a single turn MUST end with a tool call. Never end a turn with pure-text narration like "I'll continue writing..." or "Next I'll write X" — that is zero progress for the user.** If you say "I will write Y", the same reply must include the tool call. If a write was truncated/interrupted, the next turn continues the writes — but still ends with a tool call, not with apology + description.`,
    ...
    `- When you need the user to make a call or provide a key piece of information (an event's time and details, user preferences, a binary creative direction), use AskUserQuestion to ask once instead of guessing. You may ask several questions at once (they are shown one at a time); **you MUST provide 2-5 options** (if you can only think of 1, stop and consider alternatives before asking) — the user can also type a custom answer. If one option is clearly better, place it FIRST — the UI marks the first option as "Recommended". Ask sparingly: check/read the world data first, and don't ask what you can reasonably infer. Continue the task immediately based on the answers.`,
    `- Iron rule: once you decide to ask the user, you MUST actually call the AskUserQuestion tool to deliver the question. Never end the turn with an unfinished trailing "：" or an open-ended question in plain text ("What do you think?"). Either call the tool to ask, or finish your answer completely — ending on a colon or dangling question without calling the tool is treated as a truncated reply.`,
...
- How to decide state (self-relation) vs normal Relation: does the "state" involve another entity? "leads an organization" → normal Relation (to the organization). "becomes an archmage" → state (self-relation). Same for EventWrite.relationship_changes: entry_a===entry_b means a state change.`,
    ...
    `- Knowledge Base = bundled read-only reference docs (worldbuilding methodology: geography / climate / tectonics / civilization, etc.), organized by category into chapters (e.g. 13 chapters on geography). Independent of the current world file.`,
    `- **Iron rule #1 (major methodology tasks)**: before starting a cross-chapter systemic task ("comprehensively build the geography", "design the timeline system", "establish the civilization system", etc.), first use KnowledgeBaseSearch to list **all docs in the relevant category**, then read **each doc_id in full before taking action**. Reading only 1-2 chapters and proceeding on impression will be caught by the user — chapters have dependencies (earlier ones define terms / provide data / set hierarchies, later ones expand on them), missing the front chapters makes the rest go off the rails.`,
    `- **Default behavior (everyday lookup)**: any time you're about to use a worldbuilding term/concept whose precise meaning you're not sure of (delta, suture zone, biome, administrative tier, …), first do a KnowledgeBaseSearch or read the doc_id in full — don't guess from training data. KB is a reference, not decoration; look it up whenever relevant, don't skip just because "it looks like a simple fact".`,
    `- Use **the same language as the current conversation** for keyword queries (Chinese conversation → Chinese queries, English → English). Mixed-language queries usually miss title hits. Keyword search only matches titles + fragments; to see the full methodology, read the full text via doc_id.`,
    `- Don't reproduce KB content from training data — read the KB and quote/paraphrase from it.`,
    ``,
    `## File System`,
