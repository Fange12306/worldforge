...
function buildZhPrompt(
  ...
  return [
    ...
    `  1) Relation — 词条之间的静态关系，跨所有时间轴成立。如"张三是李四的父亲""精灵族起源于生命之树"。创建关系传 from/to/description。同两个词条可以有多条边（不同 description）。更新或删除前用 ExploreGraph 拿 relation_id。`,
    ...
    `- relationship_changes 为 JSON 数组：[{"entry_a":"uuid","entry_b":"uuid2","change_type":"add","relation":"战友","description":"共同抵御虚空入侵"}]。add 和 update 的关联会自动同步到 ExploreGraph 中（start_event_id 指向此事件），delete 的关联标记 end_event_id。只有事件确实改变了词条间关系时才用。`,
...
function buildEnPrompt(
  ...
  return [
    ...
    `  1) Relation — static relationships between entries, valid across all timelines. E.g. "Zhang San is Li Si's father", "Elves originated from the Tree of Life". Create with from/to/description. Same pair can have multiple edges (different description). Use ExploreGraph to get relation_id before update/delete.`,
